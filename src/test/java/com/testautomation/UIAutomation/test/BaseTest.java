...
import static com.testautomation.UIAutomation.helper.util.ExtentReport.FailureMessage;
import static com.testautomation.UIAutomation.helper.util.ExtentReport.SuccessMessage;
...
/**
 * @author tm0338 Contains methods to start and stop webdriver, reporting and
...
public class BaseTest {
	...
	public void setup() {
		...
		if (!(new BasePage(driver).PopulateElementMap())) {
			finishTestReporting(ITestResult.FAILURE,
					"Could not populate element map from excel file. See logs for detail.<br>"
	...
	public boolean loginToPortal() {
		...
		if (!(new LoginPage(driver).LoginToDemoPage("standard_user", "secret_sauce"))) {
			finishTestReporting(ITestResult.FAILURE,
					"Could not login using given credentials.<br>" + GetScreenshotForReport(driver, "Test_Setup"));
			driver.quit();
			return false;
		} else {
			finishTestReporting(ITestResult.SUCCESS,
					"Successfully logged in to website.<br>" + GetScreenshotForReport(driver, "Test_Setup"));
		}
		return true;
	}
